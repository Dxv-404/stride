...
28. [Risk Mitigation Plan](#28-risk-mitigation-plan)
...
│   ├── generate_report.py
│   └── figures/
├── web/                     # Vite + React + TypeScript interactive website
│   ├── src/
│   │   ├── engine/          # p2.js physics, GA, encoding (TypeScript ports)
│   │   ├── components/      # React UI components
│   │   ├── pages/           # Route pages (Lab, Playground, Compare, etc.)
│   │   ├── stores/          # Zustand state stores
│   │   └── lib/             # Worker bridge, utilities
│   ├── public/
│   ├── package.json
│   └── vite.config.ts
├── main.py
...
Multi-page React application (Vite + TypeScript) with live 2D physics simulation running in the browser using p2.js + PixiJS.

**Tech Stack**:
- React (JSX artifact)
- p2.js (CDN: `https://cdnjs.cloudflare.com/ajax/libs/p2.js/0.7.1/p2.min.js`) — 2D rigid body physics (closest browser equivalent to pymunk: has RevoluteConstraint with motorEnabled, angle limits, spring joints)
- PixiJS or Canvas2D — rendering (p2.js is headless, needs separate renderer)
...
**CPG Sanity Test (Day 1 — MUST PASS before proceeding)**: Run 10-generation mini-evolution. Verify best fitness > 200 by gen 10. If not: (1) reduce coupling weight range to [-1, 1], (2) initialize coupling weights near zero, (3) increase population to 150. **See Section 28 (Risk 1) for three additional mitigations: sine-seeded CPG init, phased evolution, and reduced coupling topology.**
...
**Search space warning**: 96 dimensions with 15,000 evaluations is tight. **See Section 28 (Risk 3) for mitigations**: 2× eval budget, gene-group-specific mutation rates, evaluation-normalized comparison, and CMA-ES benchmarking.
...
**All three are valid findings.** Don't debug if you get B or C — report honestly. **See Section 28 (Risk 2) for additional mitigations if the NN learns nothing**: perturbation during training, increased modulation range, reactivity bonus (last resort).
...
|---|-----------|-----------|-----------|
| 1 | CPG baseline (38 genes, flat; sine-seeded if Risk 1 triggers) | 30 runs | ~30 min |
...
| 6b | CPG+NN 2× budget (150 pop × 200 gen, if Risk 3 triggers) | 30 runs | ~90 min |
| 6c | CPG+NN perturbation-trained (if Risk 2 triggers) | 30 runs | ~60 min |
...
### Tech Stack
- **React** (JSX artifact)
- **p2.js** — 2D rigid body physics (browser equivalent of pymunk — has RevoluteConstraint with motor, angle limits, springs)
- **PixiJS or Canvas2D** — rendering (p2.js is physics-only, headless)
- **Three.js** (r128) — 3D fitness landscape, chromosome helix
- **Recharts** — interactive charts
- **Tone.js** — generative soundtrack
- **Tailwind CSS** — styling
...
p2.js creature will be qualitatively similar and closer to pymunk than other browser physics engines — both use similar constraint solvers, RevoluteConstraints with motors, and angle limits. Live simulations (Tabs 1-4) use p2.js physics. Statistical results (Tab 5) show REAL pymunk experiment data. Minor numerical differences are expected but the constraint model maps nearly 1:1.

### p2.js ↔ pymunk API Mapping

| pymunk | p2.js | Notes |
|--------|-------|-------|
| `pymunk.Space()` | `new p2.World({ gravity: [0, -981] })` | p2 uses arrays [x, y], not objects |
| `space.step(dt)` | `world.step(dt)` | Same |
| `pymunk.Body(mass, moment)` | `new p2.Body({ mass })` | p2 computes moment from shapes |
| `body.position` | `body.position` | pymunk: Vec2d; p2: [x, y] array |
| `body.velocity` | `body.velocity` | Same difference |
| `body.angle` | `body.angle` | Both in radians |
| `pymunk.Segment` (ground) | `new p2.Box()` on static body | mass=0 → static in p2 |
| `PivotJoint + RotaryLimitJoint + SimpleMotor` | `new p2.RevoluteConstraint` with `.enableMotor()` and `.setLimits()` | p2 combines all three into one constraint |
| `motor.rate = speed` | `joint.setMotorSpeed(speed)` | Direct equivalent |
| `motor.max_force` | `joint.motorMaxForce` | Direct equivalent |
| `ShapeFilter(group=-1)` | `shape.collisionGroup / collisionMask` | p2 uses bitmask filtering |
| `body.apply_impulse_at_local_point()` | `body.applyImpulse([fx, fy])` | Same effect |
| Gravity: y-down (positive = down) | Gravity: y-up (negative = down) | **CRITICAL**: flip y when rendering to canvas |
...
**Technical implementation**: Worker runs p2.js physics headlessly (p2.js works in workers — no DOM needed). Main thread renders best creature via Canvas2D or PixiJS. Camera follows creature.
...
function ControllerRace() {
  ...
  const sineWorld = useP2World(BEST_CHROMOSOMES.sine, "sine", terrain);
  const cpgWorld = useP2World(BEST_CHROMOSOMES.cpg, "cpg", terrain);
  const cpgnnWorld = useP2World(BEST_CHROMOSOMES.cpgnn_flat, "cpgnn", terrain);
  return (
...
function PushTest() {
  ...
  const handlePush = () => {
    ...
    const force = [-pushStrength, 0];  // BACKWARD (Errata Fix #1)
    torso.applyForce(force);  // p2.js body.applyForce([fx, fy])
    setTimeout(() => {
...
Small note at bottom: "Live simulations use p2.js (close to pymunk but not identical). Statistical results are from pymunk (exact)."
...
#### Feature 37: ~~Trail of Breadcrumbs Analytics~~ [DROPPED]

~~Heatmap overlay (toggle) showing visitor interaction patterns.~~ Dropped — not relevant for academic project submission.
...
Software: pymunk, matplotlib, scipy, numpy, reportlab, p2.js, PixiJS, Three.js, React, Recharts, Tone.js
...
61. Sine-seeded CPG: oscillator genes identical across population → crossover ineffective early. Ensure ±3% noise on seeded genes.
62. Perturbation-during-training: random pushes cause most creatures to fall early → lower push magnitude range (200-600) or reduce to 1 push per sim.
63. Gene-group mutation: NN genes mutated more → NN may dominate CPG changes. Monitor CPG gene stability across generations.
...
**Day 1** (8h): p2.js creature + 3 controllers + Canvas2D/PixiJS renderer. Tab 4 (Playground) with sliders. Tab 3 (Push Test) with impulse. Core is playable.
...
p2.js creature → Tab 3 (Push Test) with particles + screen shake + ghost trail → Tab 4 (Playground) → A/B comparison. ~16 hours.
...
| Task | Time | Priority |
|------|------|----------|
| p2.js creature + controllers | 4-5h | P0 |
...
A: Biological architecture argument. Real locomotion uses CPGs in the spinal cord with cortical modulation. Three-tier comparison (open-loop → coupled-oscillator → feedback-modulated) tests whether each layer of biological complexity actually helps. The frozen-NN control isolates dimensionality from feedback. The cascade initialization (sine → CPG → CPG+NN) mirrors how biological evolution builds incrementally — first rhythmic contraction, then coupled oscillation, then sensory modulation.
...
### p2.js Creature Module

```javascript
function createCreature(world, x, y, genes, controllerType) {
  // Collision group: negative means bodies in same group don't collide with each other
  const CREATURE_GROUP = -1;

  // Helper: create a rectangular body and add to world
  function makeBox(px, py, w, h, mass, label) {
    const body = new p2.Body({ mass, position: [px, py] });
    const shape = new p2.Box({ width: w, height: h });
    shape.collisionGroup = 0x0002;
    shape.collisionMask = 0x0001;  // Only collide with ground, not other creature parts
    body.label = label;
    body.addShape(shape);
    world.addBody(body);
    return body;
  ...
  // Body parts (p2.js uses center-of-mass coordinates, y-up)
  const torso    = makeBox(x, y,        60, 20, 5, "torso");
  const thighL   = makeBox(x - 15, y - 30, 12, 35, 2, "thighL");
  const shinL    = makeBox(x - 15, y - 65, 10, 30, 1.5, "shinL");
  const thighR   = makeBox(x + 15, y - 30, 12, 35, 2, "thighR");
  const shinR    = makeBox(x + 15, y - 65, 10, 30, 1.5, "shinR");
  const upperArmL = makeBox(x - 35, y + 5, 10, 25, 1, "upperArmL");
  const forearmL  = makeBox(x - 35, y - 20, 8, 20, 0.8, "forearmL");
  const upperArmR = makeBox(x + 35, y + 5, 10, 25, 1, "upperArmR");
  const forearmR  = makeBox(x + 35, y - 20, 8, 20, 0.8, "forearmR");
  const footL    = makeBox(x - 15, y - 82, 18, 6, 0.5, "footL");
  const footR    = makeBox(x + 15, y - 82, 18, 6, 0.5, "footR");

  const allBodies = [torso, thighL, shinL, thighR, shinR, upperArmL, forearmL, upperArmR, forearmR, footL, footR];

  // Motorized joints (6) — p2.RevoluteConstraint with motor
  function motorJoint(bodyA, bodyB, pivotA, pivotB, limits) {
    const joint = new p2.RevoluteConstraint(bodyA, bodyB, {
      localPivotA: pivotA,
      localPivotB: pivotB,
    ...
    joint.motorMaxForce = 50;  // Tunable: higher = stronger joints
    if (limits) {
  ...
  // Passive joints (elbows + ankles) — no motor
  function passiveJoint(bodyA, bodyB, pivotA, pivotB, limits) {
    const joint = new p2.RevoluteConstraint(bodyA, bodyB, {
      localPivotA: pivotA,
      localPivotB: pivotB,
      collideConnected: false,
  ...
  const hipL      = motorJoint(torso, thighL,    [-15, -10], [0, 17],  [-Math.PI/2, Math.PI/2]);
  const hipR      = motorJoint(torso, thighR,    [15, -10],  [0, 17],  [-Math.PI/2, Math.PI/2]);
  const kneeL     = motorJoint(thighL, shinL,    [0, -17],   [0, 15],  [-Math.PI/2, 0]);
  const kneeR     = motorJoint(thighR, shinR,    [0, -17],   [0, 15],  [-Math.PI/2, 0]);
  const shoulderL = motorJoint(torso, upperArmL, [-30, 5],   [0, 12],  [-Math.PI/2, Math.PI/2]);
  const shoulderR = motorJoint(torso, upperArmR, [30, 5],    [0, 12],  [-Math.PI/2, Math.PI/2]);

  const elbowL = passiveJoint(upperArmL, forearmL, [0, -12], [0, 10], [-Math.PI/3, 0]);
  const elbowR = passiveJoint(upperArmR, forearmR, [0, -12], [0, 10], [-Math.PI/3, 0]);
  const ankleL = passiveJoint(shinL, footL, [0, -15], [3, 0], [-Math.PI/6, Math.PI/6]);
  const ankleR = passiveJoint(shinR, footR, [0, -15], [3, 0], [-Math.PI/6, Math.PI/6]);

  const motorJoints = [hipL, hipR, kneeL, kneeR, shoulderL, shoulderR];
  const allConstraints = [...motorJoints, elbowL, elbowR, ankleL, ankleR];
  ...
  return {
    torso, allBodies, allConstraints, controller, motorJoints,
    // Update motor speeds each frame to track target angles
    update(t) {
      const sensors = this.getSensors();
      const targets = controllerType === "cpgnn"
        ? controller.getTargets(t, sensors)
        : controller.getTargets(t);
      // PD control: motor speed proportional to angle error
      const kP = 20.0;
      motorJoints.forEach((joint, i) => {
        const currentAngle = joint.bodyB.angle - joint.bodyA.angle;
        const error = targets[i] - currentAngle;
        joint.setMotorSpeed(kP * error);
      });
    },
    getDistance: () => torso.position[0] - startX,
    getSensors: () => {
      const torsoAngle = torso.angle;
      const torsoAngVel = torso.angularVelocity;
      const groundY = -400;  // adjust to your ground position
      const footLDown = footL.position[1] < groundY + 15;
      const footRDown = footR.position[1] < groundY + 15;
      const height = torso.position[1] - groundY;
      const hVel = torso.velocity[0];
...
class CPGController {
  ...
  getTargets(t, dt) { return this.step(dt || 1/60); }
}
...
### Terrain Creation (p2.js)

```javascript
function createTerrain(world, type, width = 5000) {
  const GROUND_Y = 0;  // p2.js: y-up coordinate system
  const groundMaterial = new p2.Material();

  if (type === "flat") {
    const ground = new p2.Body({ mass: 0, position: [width / 2, GROUND_Y] });  // mass=0 → static
    const shape = new p2.Box({ width: width, height: 20 });
    shape.material = groundMaterial;
    shape.collisionGroup = 0x0001;
    shape.collisionMask = 0xFFFF;
    ground.addShape(shape);
    world.addBody(ground);
    return { bodies: [ground], getHeight: (x) => GROUND_Y + 10 };
  }

  if (type === "hill") {
    const bodies = [];
    // Flat sections
    const flatL = new p2.Body({ mass: 0, position: [150, GROUND_Y] });
    flatL.addShape(new p2.Box({ width: 300, height: 20 }));
    world.addBody(flatL); bodies.push(flatL);

    const flatR = new p2.Body({ mass: 0, position: [width / 2 + 250, GROUND_Y] });
    flatR.addShape(new p2.Box({ width: width - 500, height: 20 }));
    world.addBody(flatR); bodies.push(flatR);

    // Hill: line segments approximating sine curve
    for (let i = 0; i < 50; i++) {
      const x1 = 300 + i * 4;
      const x2 = 300 + (i + 1) * 4;
      const y1 = GROUND_Y + 50 * Math.sin(Math.PI * (x1 - 300) / 200);
      const y2 = GROUND_Y + 50 * Math.sin(Math.PI * (x2 - 300) / 200);
      const midX = (x1 + x2) / 2;
      const midY = (y1 + y2) / 2;
      const len = Math.sqrt((x2 - x1) ** 2 + (y2 - y1) ** 2);
      const angle = Math.atan2(y2 - y1, x2 - x1);
      const seg = new p2.Body({ mass: 0, position: [midX, midY], angle: angle });
      seg.addShape(new p2.Box({ width: len, height: 4 }));
      world.addBody(seg); bodies.push(seg);
    }

    return {
      bodies,
      getHeight: (x) => {
        if (x >= 300 && x <= 500) return GROUND_Y + 50 * Math.sin(Math.PI * (x - 300) / 200) + 10;
        return GROUND_Y + 10;
      }
    };
  }
  ...
  return createTerrain(world, "flat", width);
}
```

### Custom Canvas Renderer (for p2.js)

```javascript
function drawCreature(ctx, creature, cameraX, canvasW, canvasH) {
  ctx.clearRect(0, 0, canvasW, canvasH);
  // Background
  ctx.fillStyle = "#111827";
  ctx.fillRect(0, 0, canvasW, canvasH);

  // p2.js uses y-up; canvas uses y-down. Transform:
  // screenX = worldX - cameraX + canvasW/2
  // screenY = canvasH/2 - worldY  (flip y)
  const toScreenX = (wx) => wx - cameraX + canvasW / 2;
  const toScreenY = (wy) => canvasH / 2 - wy;

  // Ground line
  ctx.strokeStyle = "#4ADE80";
  ctx.lineWidth = 2;
  ctx.beginPath();
  ctx.moveTo(0, toScreenY(10));
  ctx.lineTo(canvasW, toScreenY(10));
  ctx.stroke();
  ...
  for (let d = 0; d < 5000; d += 100) {
    const sx = toScreenX(d);
    if (sx > 0 && sx < canvasW) {
      ctx.fillText(`${d}px`, sx, toScreenY(10) + 15);
      ctx.beginPath();
      ctx.moveTo(sx, toScreenY(10) - 2);
      ctx.lineTo(sx, toScreenY(10) + 2);
      ctx.stroke();
    }
  }
  ...
  creature.allBodies.forEach(body => {
    const shape = body.shapes[0];  // Each body has one Box shape
    const w = shape.width, h = shape.height;
    const [bx, by] = body.position;
    const angle = body.angle;

    ctx.save();
    ctx.translate(toScreenX(bx), toScreenY(by));
    ctx.rotate(-angle);  // negate because canvas y is flipped
    ctx.fillStyle = COLORS[body.label] || "#94A3B8";
    ctx.fillRect(-w / 2, -h / 2, w, h);
    ctx.restore();
  });

  // Joint circles at constraint pivot points
  creature.motorJoints.forEach(joint => {
    const [ax, ay] = joint.bodyA.position;
    const [px, py] = joint.pivotA;  // local pivot
    const cos = Math.cos(joint.bodyA.angle);
    const sin = Math.sin(joint.bodyA.angle);
    const worldX = ax + cos * px - sin * py;
    const worldY = ay + sin * px + cos * py;
    ctx.beginPath();
    ctx.arc(toScreenX(worldX), toScreenY(worldY), 4, 0, Math.PI * 2);
...
## 28. RISK MITIGATION PLAN

Three critical risks that could derail the v2 additions, with concrete mitigations for each.

### Risk 1: CPG Sanity Test May Fail

**The worry**: 38 genes with coupled oscillators is a harder landscape than 18 independent sine genes. The GA might not find good CPG walkers within 150 generations.

**Why this could happen**: Coupling weights create dependencies between joints. A good hip frequency with bad coupling weights produces worse walking than no coupling at all. The GA has to simultaneously discover good intrinsic parameters AND good coupling — a chicken-and-egg problem. Early generations have random coupling that actively fights the oscillators.

#### Mitigation 1A: Sine-Seeded CPG Initialization

Seed CPG from the best sine, just as CPG+NN is seeded from the best CPG. This creates a cascade: **sine → CPG → CPG+NN**.

```python
def initialize_cpg_population(pop_size, best_sine_chromosomes):
    """Seed CPG population from pre-evolved sine parameters."""
    population = []
    for i in range(pop_size):
        chromosome = np.zeros(38)
        # Pick a random good sine chromosome as template
        sine = random.choice(best_sine_chromosomes).copy()

        # Oscillator params: copy directly from sine (same gene meaning)
        chromosome[0:18] = sine[0:18]
        chromosome[0:18] += np.random.normal(0, 0.03, 18)
        chromosome[0:18] = np.clip(chromosome[0:18], 0, 1)

        # Coupling weights: near zero (gene 0.5 maps to weight 0.0 in [-2, 2])
        chromosome[18:38] = np.random.normal(0.5, 0.02, 20)
        chromosome[18:38] = np.clip(chromosome[18:38], 0, 1)

        population.append(chromosome)
    return population
```

Creatures already walk via sine-like oscillators; GA only needs to discover useful coupling on top. Coupling starts near zero (neutral) — can't make things worse initially.

#### Mitigation 1B: Phased Evolution

Two-phase approach — freeze coupling for first 50 generations:

```python
def phased_cpg_evolution(config):
    """Phase 1: Oscillator params only. Phase 2: Unlock coupling."""
    # Phase 1: 50 gen, coupling genes FROZEN at 0.5 (zero coupling)
    config_phase1 = {**config, "max_generations": 50, "frozen_genes": range(18, 38)}
    results_phase1 = run_ga(config_phase1)

    # Phase 2: 100 gen, all 38 genes unlocked
    config_phase2 = {**config, "max_generations": 100, "frozen_genes": []}
    results_phase2 = run_ga(config_phase2, initial_population=results_phase1.final_population)

    return merge_results(results_phase1, results_phase2)
```

Phase 1 is essentially sine evolution. Phase 2 starts from walking creatures and discovers coupling.

#### Mitigation 1C: Reduced Coupling Topology

If convergence is slow, reduce from 10 to 4 biologically critical connections:

- hip_L ↔ hip_R (contralateral: walking requires antiphase hips)
- hip_L → knee_L, hip_R → knee_R (ipsilateral: knee follows hip)

Drops from 20 coupling genes to 8 (total 26 genes instead of 38). Shoulder coupling sacrificed, but hip-knee coordination and left-right alternation preserved.

**Cascade insight for the report**: The sine → CPG → CPG+NN progression mirrors how biological evolution actually works — first rhythmic contraction, then coupled oscillation, then sensory modulation. Each layer optimized before the next is added. This is itself a finding worth discussing.

---

### Risk 2: CPG+NN Might Not Use the NN

**The worry**: If CPG alone produces excellent walking on flat terrain, there's no evolutionary pressure for the NN to learn anything. NN weights stay near initialization (≈0 modulation), sensor ablation shows no effect, frozen-NN equals active-NN, and the reactive control story falls flat.

**Why this is likely on flat terrain**: Flat terrain is perfectly predictable. A well-tuned open-loop controller can walk forever without sensing anything. The GA, being lazy and efficient, won't optimize 58 NN weights when 38 CPG weights already produce good fitness.

#### Mitigation 2A: Train CPG+NN on Mixed Terrain (Already in Spec)

Make mixed-terrain the PRIMARY CPG+NN experiment, not flat-trained. On mixed terrain, the CPG alone can't handle unpredictability — the NN becomes necessary because sensor inputs carry terrain-specific information the CPG can't incorporate.

If flat-trained CPG+NN equals frozen-NN, that's actually a valid finding: "On predictable terrain, sensory feedback provides no advantage."

#### Mitigation 2B: Add Perturbation During Training

Force the NN to be useful by making the training environment unpredictable:

```python
def evaluate_with_perturbations(chromosome, terrain, config):
    """Training evaluation with random perturbations to force NN learning."""
    sim = setup_simulation(chromosome, controller_type="cpg_nn", config=config)

    # Apply 2-3 random pushes during 15-second simulation
    push_times = sorted(random.sample(range(180, 720), 2))  # t=3s to t=12s
    push_magnitude = random.uniform(300, 1000)

    for step in range(900):
        if step in push_times:
            direction = random.choice([-1, 1])
            sim.creature.torso.apply_impulse_at_local_point(
                (direction * push_magnitude, 0), (0, 0)
            )
        sim.step()
...
The key insight: if you only train on a perfectly stable environment, there's no selection pressure for stability. You need instability in training to evolve stability.

#### Mitigation 2C: Increase Modulation Range

If NN isn't learning with ±50% modulation, increase to ±100%:

```python
final_targets = [cpg * (1.0 + 1.0 * mod) for cpg, mod in zip(cpg_targets, m)]
```

Now the NN can fully silence a joint (mod=-1.0 → output=0) or double it (mod=1.0 → output=2×). More authority = worth the GA's effort to optimize. Tradeoff: more damage if NN weights are bad, which is why CPG-seeded init matters.

#### Mitigation 2D: Reactivity Bonus (Last Resort)

```python
def compute_reactivity_bonus(nn_modulation_history):
    """Small bonus for NN utilization. Use ONLY if NN learns nothing organically."""
    modulation_array = np.array(nn_modulation_history)  # shape: (900, 6)
    temporal_variance = np.mean(np.var(modulation_array, axis=0))
    return min(temporal_variance * 20.0, 15.0)  # capped at 15 units
```

**Warning**: Artificial incentive. Disclose in methodology: "A small reactivity bonus (max 15 units) was added to encourage NN utilization." Only use if mitigations 2A/2B fail.

**Environment shapes the organism**: If the environment is too easy, evolution finds the simplest solution (ignore NN). To discover complex control strategies, the environment must demand them. This is exactly why biological CPGs evolved sensory modulation.

---

### Risk 3: The 96-Gene Search Space Is Tight

**The worry**: 100 pop × 150 gen = 15,000 evaluations for 96 dimensions. Theory suggests O(n log n) evaluations per dimension → ~600,000 needed. At 15,000, the GA explores ~2.5% of what theory recommends.

**Why this matters**: GA might find mediocre CPG+NN and declare it "the best" when far better solutions exist. Comparison unfair: sine had 833 evals/dimension (15,000/18) while CPG+NN had only 156.

#### Mitigation 3A: Increase Budget for CPG+NN

```python
CPGNN_CONFIG = {
    **BASELINE_CONFIG,
    "population_size": 150,    # 50% larger
    "max_generations": 200,    # 33% more
    # Total: 30,000 evaluations (2× baseline)
}
```

30,000 evals gives ~312 evals/dimension. ~90 min for 30 runs (vs ~45 min). Report transparently: "CPG+NN was given 2× evaluation budget to partially compensate for larger search space."

#### Mitigation 3B: Evaluation-Normalized Comparison

Plot convergence with x-axis = total evaluations, not generations. Two arguments:

1. At equal budget (15,000 evals): fair comparison
2. At extended budget (30,000 evals): CPG+NN gets chance to show potential

If CPG+NN at 15,000 evals already beats sine → strong result. If only beats at 30,000 → valid cost-performance tradeoff finding.

#### Mitigation 3C: Gene-Group-Specific Mutation Rates

```python
def mutate_cpgnn(chromosome, gen, max_gen):
    """Differentiated mutation rates for CPG+NN gene groups."""
    mutated = chromosome.copy()

    # Oscillator genes (0-17): low mutation — well-seeded from CPG
    for i in range(18):
        if random.random() < 0.03:
            mutated[i] += random.gauss(0, 0.05)

    # Coupling genes (18-37): medium mutation — explore coordination
    for i in range(18, 38):
        if random.random() < 0.05:
            mutated[i] += random.gauss(0, 0.1)

    # NN weights (38-95): higher mutation — most to learn
    for i in range(38, 96):
        if random.random() < 0.08:
            mutated[i] += random.gauss(0, 0.15)

    return np.clip(mutated, 0, 1)
```

Allocates more mutation budget to NN genes (most exploration needed) and less to oscillator genes (already good from seeding). A form of prior knowledge injection.

#### Mitigation 3D: Uniform Crossover (Already in Spec)

Already specified for CPG+NN. Single-point on 96 genes usually cuts within one gene group. Uniform crossover mixes parent A's CPG with parent B's NN — testing whether one parent's walking style works with the other's balance reflexes.

#### Mitigation 3E: CMA-ES Comparison on CPG+NN
...
CPGNN_CMAES_CONFIG = {
    **CPGNN_CONFIG,
    "algorithm": "cmaes",
    "cmaes_sigma": 0.3,
    # 96 genes is right in CMA-ES's sweet spot (10-100 dims)
}
```

CMA-ES adapts its search distribution using covariance of successful mutations — theoretically better for smooth high-dimensional landscapes. If CMA-ES significantly outperforms GA on CPG+NN, the landscape is smooth and crossover isn't helping. If GA matches CMA-ES, crossover contributes useful recombination. Strengthens the report either way.

**Dimensionality insight for report**: The tension between controller expressiveness and evaluation budget is a fundamental problem in evolutionary robotics. That GA finds anything useful in 96D with 15,000 evaluations suggests exploitable landscape structure — which FDC and epistasis analysis can characterize.
...
|------|-------------------|----------|
| CPG won't converge | Sine-seeded initialization (cascade: sine→CPG→CPG+NN) | Phased evolution (freeze coupling first 50 gen) |
| NN not used | Train on mixed terrain + random perturbations | Increase modulation range to ±100% |
| 96-gene space too large | 2× eval budget + gene-group-specific mutation | CMA-ES comparison to benchmark GA effectiveness |

**The cascade initialization (sine → CPG → CPG+NN) is the single most impactful mitigation** because it addresses all three risks simultaneously: CPG starts from working sine oscillators (won't fail), CPG+NN starts from working CPG (NN only learns modulation), and both tiers start in good search space regions (less wasted exploration).
